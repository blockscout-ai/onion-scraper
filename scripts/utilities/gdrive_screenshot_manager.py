...
class GoogleDriveScreenshotManager:
    ...
    def file_exists_in_drive(self, local_file_path):
        """
        Check if a file with the same name or MD5 hash exists in Google Drive (in the correct folder).
        ...
        entity_id = self._generate_entity_id(local_file_path)
        entity_folder_id = self._get_or_create_entity_folder(entity_id)
        # Search for file by name in the entity folder
        query = f"name='{file_name}' and '{entity_folder_id}' in parents and trashed=false"
        try:
            results = self.drive_service.files().list(q=query, spaces='drive', fields='files(id, name, md5Checksum)').execute()
            files = results.get('files', [])
            for f in files:
                # If hash matches, it's a confirmed match
                if 'md5Checksum' in f and f['md5Checksum'] == file_hash:
                    return True
                # If no hash, but name matches, assume match (conservative)
                if f['name'] == file_name:
                    return True
            return False
        except Exception as e:
            print(f"⚠️ Error checking file existence in Drive: {e}")
            return False
