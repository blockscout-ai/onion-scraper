...
def cleanup_after_verification(screenshot_dir="screenshots_fast", days_old=1):
    ...
    Only deletes files older than specified days to ensure they were uploaded.
    
    Args:
        screenshot_dir (str): Directory containing screenshots
        days_old (int): Only delete files older than this many days
    """
    print("🧹 Starting cleanup of local screenshots...")
    print(f"📁 Directory: {screenshot_dir}")
    print(f"⏰ Only deleting files older than {days_old} day(s)")
    print()
    ...
    manager.cleanup_old_local_screenshots(screenshot_dir, days_old=days_old)
